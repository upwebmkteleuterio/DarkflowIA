...
const GlobalBatchProgress: React.FC = () => {
  const { state, clearQueue } = useBatch();
  const { stats, isProcessing, tasks, currentTaskId } = state;

  if (tasks.length === 0) return null;

  const currentTask = tasks.find(t => t.id === currentTaskId);
  const isFinished = stats.completed + stats.failed === stats.total;
        ...
        <div className="p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className={`size-10 rounded-xl flex items-center justify-center ${isFinished ? 'bg-accent-green/20 text-accent-green' : 'bg-primary/20 text-primary border border-primary/20'}`}>
              ...
              <div className="text-left">
                ...
                <p className="text-xs font-bold text-white">
                  {stats.completed} de {stats.total} finalizados
                </p>
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="w-full h-2 bg-background-dark rounded-full border border-border-dark overflow-hidden">
              <div 
                className={`h-full transition-all duration-1000 ${isFinished ? 'bg-accent-green shadow-[0_0_10px_#39FF14]' : 'bg-primary shadow-[0_0_10px_#8655f6]'}`}
